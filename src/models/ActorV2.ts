...
import type { APIKeyV2 } from './APIKeyV2';
import {
    APIKeyV2FromJSON,
    APIKeyV2FromJSONTyped,
    APIKeyV2ToJSON,
} from './APIKeyV2';
...
/**
 * Check if a given object implements the ActorV2 interface.
 */
export function instanceOfActorV2(value: object): boolean {
    let isInstance = true;

    return isInstance;
}

export function ActorV2FromJSON(json: any): ActorV2 {
    return ActorV2FromJSONTyped(json, false);
}

export function ActorV2FromJSONTyped(json: any, ignoreDiscriminator: boolean): ActorV2 {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'api_key': !exists(json, 'api_key') ? undefined : APIKeyV2FromJSON(json['api_key']),
        'user': !exists(json, 'user') ? undefined : UserV1FromJSON(json['user']),
    };
}

export function ActorV2ToJSON(value?: ActorV2 | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'api_key': APIKeyV2ToJSON(value.api_key),
        'user': UserV1ToJSON(value.user),
    };
}

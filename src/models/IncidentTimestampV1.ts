 ...
 * 
 * @export
 * @interface IncidentTimestampV1
 */
export interface IncidentTimestampV1 {
    /**
     * When this last occurred, if it did
     * @type {Date}
     * @memberof IncidentTimestampV1
     */
...
/**
 * Check if a given object implements the IncidentTimestampV1 interface.
 */
export function instanceOfIncidentTimestampV1(value: object): boolean {
    let isInstance = true;
...
export function IncidentTimestampV1FromJSON(json: any): IncidentTimestampV1 {
    return IncidentTimestampV1FromJSONTyped(json, false);
}

export function IncidentTimestampV1FromJSONTyped(json: any, ignoreDiscriminator: boolean): IncidentTimestampV1 {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'last_occurred_at': !exists(json, 'last_occurred_at') ? undefined : (new Date(json['last_occurred_at'])),
        'name': json['name'],
    };
}

export function IncidentTimestampV1ToJSON(value?: IncidentTimestampV1 | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'last_occurred_at': value.last_occurred_at === undefined ? undefined : (value.last_occurred_at.toISOString()),
        'name': value.name,
    };
}

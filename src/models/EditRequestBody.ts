...
import type { IncidentEditPayloadV2 } from './IncidentEditPayloadV2';
import {
    IncidentEditPayloadV2FromJSON,
    IncidentEditPayloadV2FromJSONTyped,
    IncidentEditPayloadV2ToJSON,
} from './IncidentEditPayloadV2';
...
/**
 * Check if a given object implements the EditRequestBody interface.
 */
export function instanceOfEditRequestBody(value: object): boolean {
    let isInstance = true;
    isInstance = isInstance && "incident" in value;
    isInstance = isInstance && "notify_incident_channel" in value;

    return isInstance;
}

export function EditRequestBodyFromJSON(json: any): EditRequestBody {
    return EditRequestBodyFromJSONTyped(json, false);
}

export function EditRequestBodyFromJSONTyped(json: any, ignoreDiscriminator: boolean): EditRequestBody {
    if ((json === undefined) || (json === null)) {
        return json;
    }
    return {
        
        'incident': IncidentEditPayloadV2FromJSON(json['incident']),
        'notify_incident_channel': json['notify_incident_channel'],
    };
}

export function EditRequestBodyToJSON(value?: EditRequestBody | null): any {
    if (value === undefined) {
        return undefined;
    }
    if (value === null) {
    ...
    return {
        ...
        'incident': IncidentEditPayloadV2ToJSON(value.incident),
        'notify_incident_channel': value.notify_incident_channel,
    };
}
